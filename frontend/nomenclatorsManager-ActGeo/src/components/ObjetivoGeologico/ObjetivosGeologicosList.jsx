...
import ObjetivosDataService from "../../services/ObjetivosGeologicosServices";
...
export default function ObjetivosGeologicosList() { 
    ...
    const retrieveObjetivos = () => {
    ObjetivosDataService.getAll()
      .then(response => {
    ...
    return (
     <div class="main">
        ...
        <div class="gp83">
            ...
            <Heading>Listado de Objetivos Geológicos</Heading>
          <section>
            { !objetivos.length ===0 ? (<div>Loading ...</div>) : (
              objetivos.map((objetivo, index) => (
              <Box key={objetivo._id}
                bg="gray.100"
                p={4}
                m={4}
                borderRadius="lg"
              >
                <Flex align="center">
                  ...
                  <Text fontSize="2x1">
                      Objetivo Geológico {index + 1} : <strong>{objetivo.nombre}</strong>
                  </Text>
                </Flex>
              </Box>)
            ))}  
