...
import ProyectosDataService from "../../services/TipoProyectoGeologico";
...
export default function TiposProyectoGeologicosList() { 
    ...
    const retrieveProyectos = () => {
    ProyectosDataService.getAll()
      .then(response => {
    ...
    return (
     <div class="main">
        ...
        <div class="gp83">
            ...
            <Heading>Listado de Tipos de Proyectos Geológicos</Heading>
          <section>
            { !proyectos.length ===0 ? (<div>Loading ...</div>) : (
              proyectos.map((proyecto, index) => (
              <Box key={proyecto._id}
                bg="gray.100"
                p={4}
                m={4}
                borderRadius="lg"
              >
                <Flex align="center">
                  ...
                  <Text fontSize="2x1">
                      Tipo de proyecto {index + 1} : <strong>{proyecto.nombre}</strong>
                  </Text>
                </Flex>
              </Box>)
            ))}  
